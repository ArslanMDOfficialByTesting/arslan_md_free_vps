...
// Constants
const OFFICIAL_REPO = 'Arslan-MD/Arslan_MD';
const HEROKU_API = {
...
function isValidSession(session_id) {
  try {
    return (
      (session_id.startsWith("(ARSLAN-MD~)") || session_id.startsWith("ARSLAN-MD~")) &&
      Buffer.from(session_id.replace(/^(\(ARSLAN-MD~\)|ARSLAN-MD~)/, ''), 'base64').length > 20
    );
  } catch {
    return false;
  }
  ...
  try {
    ...
    if (!isVerified) {
      return res.status(403).json({
        ...
        steps: [
          "1. Fork https://github.com/Arslan-MD/Arslan_MD",
    ...
    if (!isValidSession(session_id)) {
      return res.status(400).json({
        ...
        required_format: "(ARSLAN-MD~)base64_encoded_string",
        example: "(ARSLAN-MD~)eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
      });
    }
    ...
    await axios.post(`${HEROKU_API.baseURL}/apps/${APP_NAME}/builds`, {
      source_blob: {
        url: `https://github.com/${github_username}/Arslan_MD/tarball/main`
      }
    }, { headers: HEROKU_API.headers });
...
setInterval(async () => {
  try {
    ...
    const cleanupTasks = apps.map(async (app) => {
      if (app.name.startsWith('arslan-md-')) {
